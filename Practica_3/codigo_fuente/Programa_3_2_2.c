...
int search_num_R(int A[], int n, int incio, int final, int *cont);//Declaracion de la funcion para hacer la busqueda de un elemento en un arreglo dada su division en trees bloques

int main (){
	int i, num;
	int cont;
	FILE *pf = fopen("prac3_2_2.csv","at");// abrir un archivo .csv
	for(i=10; i<=MAX; i=i+10){//Los casos prueba tiene un tamano de problema dentro de un rango de 10 a 10000
		int A[i];
		...
		print_array(A,TAM);
		printf("%d\t%d\n\n", search_num_R(A,num,0,TAM-1, &cont), num);
...
int search_num_R(int A[], int n, int inicio, int final, int *cont){
	int i, j;
	(*cont)++;
	(*cont)++;
	(*cont)+=10;
	...
	if(inicio>final){//Si el valor del elemento inicial es mayor al elemento final el programa termina 
		(*cont)++;
		(*cont)++;
		return -1;
	}
	...
	if(n<A[i]){//Se ve si el elemento que se busca es menor a el valor del elemento de la posicion i
		(*cont)++;
		(*cont)++;
		return search_num_R(A, n, inicio, i-1, cont);
	}
	if(n>A[j]){//Se ve si el elemento que se busca es mayor a el valor del elemento de la posicion j
		(*cont)++;
		(*cont)++;
		return search_num_R(A, n, j+1, final, cont);
	}
	else{//Se ve si el elemento que se busca es mayor a el valor del elemento de la posicion i y menor a el valor del elemento de la posicion j
		(*cont)++;
		(*cont)++;
		return search_num_R(A, n, i+1, j-1, cont);
	}	
}
